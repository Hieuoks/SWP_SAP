...
const RegisterForm = () => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await fetch(
          "http://localhost:9999/api/v1/users/check-email",
          {
            method: "POST",
            headers: {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:9999/api/v1/users/check-email-for-google-login",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      setErrors({
        form: "An error occurred during Google Sign-In. Please try again later.",
      });
    }
  };
